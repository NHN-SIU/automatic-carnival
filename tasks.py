...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_PRAKSIS_NHN_NAUTOBOT_xxx
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.praksis_nhn_nautobot.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.praksis_nhn_nautobot.compose_http_timeout,
        "NAUTOBOT_VER": context.praksis_nhn_nautobot.nautobot_ver,
        "PYTHON_VER": context.praksis_nhn_nautobot.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.praksis_nhn_nautobot.project_name}",
        f'--project-directory "{context.praksis_nhn_nautobot.compose_dir}"',
    ]

    for compose_file in context.praksis_nhn_nautobot.compose_files:
        compose_file_path = os.path.join(context.praksis_nhn_nautobot.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.praksis_nhn_nautobot.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.praksis_nhn_nautobot.python_ver}...")
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.praksis_nhn_nautobot.python_ver
    dockerfile_path = os.path.join(context.praksis_nhn_nautobot.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=False):
    ...
    if constrain_nautobot_ver:
        ...
        if constrain_python_ver:
            command += f" --python {context.praksis_nhn_nautobot.python_ver}"
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.praksis_nhn_nautobot.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {context.praksis_nhn_nautobot.python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations praksis_nhn_nautobot"
...
@task(
    help={
        "version": "Version of Praksis NHN Nautobot to generate the release notes for.",
    }
)
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'praksis_nhn_nautobot/*' --omit *migrations*"
