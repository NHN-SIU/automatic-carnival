...
def generate_connection_data():
    ...
    desired_delivery_date = generate_random_date(order_date_dt + timedelta(days=30), order_date_dt + timedelta(days=90))
    ...
    if status == "Active":
        ...
        actually_delivery_date = generate_random_date(
            order_date_dt + timedelta(days=20), desired_delivery_date_dt + timedelta(days=10)
        )
        ...
        install_date = generate_random_date(actually_delivery_date_dt, actually_delivery_date_dt + timedelta(days=3))

        # Ensure live_date is after install_date
        install_date_dt = datetime.strptime(install_date, "%Y-%m-%dT%H:%M:%SZ")
        live_date = generate_random_date(install_date_dt, install_date_dt + timedelta(days=2))
    ...
    # For planned connections
        ...
        created_at = order_date
        updated_at = generate_random_date(datetime.strptime(created_at, "%Y-%m-%dT%H:%M:%SZ"), now)
